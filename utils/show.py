...
LastEditTime: 2023-02-15 17:23:44
'''
import os
import cv2
import math
...
def show_training2(iters, img, label, pred, img_tgt_aug, pred_tgt_aug, pred_proto, img_tgt, pred_tgt, pred_tgt_pl, save_path, tag='d'):
    img = convert2png(img)
    label = convert2png(label)
    pred = torch.argmax(pred, dim=0).float()
    pred = convert2png(pred)
    concat1 = np.concatenate([img, label, pred], axis=1)
    img_tgt_aug = convert2png(img_tgt_aug)
    pred_tgt_aug = torch.argmax(pred_tgt_aug, dim=0).float()
    pred_tgt_aug = convert2png(pred_tgt_aug)
    pred_proto = torch.argmax(pred_proto, dim=0).float()
    pred_proto = convert2png(pred_proto)
    concat2 = np.concatenate([img_tgt_aug, pred_tgt_aug, pred_proto], axis=1)
    img_tgt = convert2png(img_tgt)
    pred_tgt = torch.argmax(pred_tgt, dim=0).float()
    pred_tgt = convert2png(pred_tgt)
    pred_tgt_pl = convert2png(pred_tgt_pl)
    concat3 = np.concatenate([img_tgt, pred_tgt, pred_tgt_pl], axis=1)
