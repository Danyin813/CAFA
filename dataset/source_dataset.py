...
LastEditTime: 2023-02-11 19:18:39
'''
import os
import sys
import torch
...
class sourceDataSet(data.Dataset):
    def __init__(self, root_img, root_label, list_path, target_path=None, crop_size=(512, 512), stride=1, strong_aug=True):
    ...
    def __getitem__(self, index):
        ...
        # hist match
        if self.target_path is not None:
            img_tgt = np.asarray(Image.open(osp.join(self.target_path, self.target_list[random.randint(0, len(self.target_list)-1)])), dtype=np.uint8)
            current_img = exposure.match_histograms(current_img, img_tgt, multichannel=True)
            aux_img = exposure.match_histograms(aux_img, img_tgt, multichannel=True)
        ...
        if self.strong_aug:
            if random.uniform(0, 1) < 0.5:
                if random.uniform(0, 1) < 0.5:
                    current_img = add_gauss_noise(current_img)
                if random.uniform(0, 1) < 0.5:
                    current_img = add_gauss_blur(current_img)
                if random.uniform(0, 1) < 0.5:
                    current_img = add_mask(current_img)
            if random.uniform(0, 1) < 0.5:
                if random.uniform(0, 1) < 0.5:
                    aux_img = add_gauss_noise(aux_img)
                if random.uniform(0, 1) < 0.5:
                    aux_img = add_gauss_blur(aux_img)
                if random.uniform(0, 1) < 0.5:
                    aux_img = add_mask(aux_img)
