...
class targetDataSet_val_weih(data.Dataset):
    def __init__(self, root_img, root_label, list_path, max_iters=None, crop_size=[512, 512]):
        ...
        if 'lucchi' in self.root_img or 'SNEMI3D' in self.root_img :
            self.padding_x = 176
            self.padding_y = 48
        else:
    ...
    def __getitem__(self, index):
        ...
        # padding
        image_as_np = np.pad(image_as_np, ((self.padding_x, self.padding_x), (self.padding_y, self.padding_y)),
                             mode='reflect')
        label_as_img = np.pad(label_as_img, ((self.padding_x, self.padding_x), (self.padding_y, self.padding_y)),
                              mode='reflect')
...
#     def __init__(self, root_label, list_path, crop_size=[512, 512]):
#         self.img_ids = [i_id.strip() for i_id in open(list_path)]
#         self.labels = []
#         self.length = len(self.img_ids)
...
class targetDataSet_val_twoimgs(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # padding
        current_img = np.pad(current_img, ((self.padding_x, self.padding_x), (self.padding_y, self.padding_y)),
                             mode='reflect')
